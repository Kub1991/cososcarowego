...
import { getSmartMatchRecommendations, getMovieRecommendation, getOrCreateDefaultListId, addMovieToList, markMovieAsWatched, UserPreferences, MovieRecommendation } from '../lib/supabase';
...
const SmartMatchScreen: React.FC<SmartMatchScreenProps> = ({ onBack, isAuthenticated, onAuthPrompt, onGoToJourney }) => {
  ...
  const handleAddToList = async (movieId?: string) => {
    ...
    try {
      ...
      // Add movie to list
      const success = await addMovieToList(listId, targetMovieId);
      if (success) {
        const movieTitle = movieId ? 
          (recommendations.find(r => r.movie.id === movieId)?.movie.title || 'Film') : 
          (selectedMovieForBrief?.movie.title || 'Film');
        setActionFeedback({ type: 'success', message: `"${movieTitle}" dodano do listy!` });
      } else {
          ...
          {currentStep === 'results' && (
            <div className="space-y-6 md:space-y-8">
                      ...
                      <div className="md:col-span-3 space-y-3 md:space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
                          ...
                          >
                            ...
                            <span className="hidden sm:inline">Dodaj do listy</span>
              ...
              <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 md:gap-4 px-2">
                ...
                >
                  <img src="/moja-podroz.png" alt="Moja podróż" className="w-3 h-3 md:w-4 md:h-4" />
                  <span className="hidden sm:inline">Moja Oscar podróż</span>
                  <span className="sm:hidden">Podróż</span>
                </button>
